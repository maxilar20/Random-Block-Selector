...
class Slider(Frame):
    ...
    def __init__(
        ...
        self.canv.bind("<1>", self._leftClick)
    ...
    def refresh(self):
        for bar in self.bars:
            ids = bar["Ids"]
            bar["Pos"] = (bar["Value"] - self.min_val) / (self.max_val - self.min_val)
            for id in ids:
                self.canv.delete(id)
            bar["Ids"] = self.__addBar(bar["Pos"])
    ...
    def _mouseMotion(self, event):
        ...
        if self.slide_func is not None:
            self.slide_func(None)
    ...
    def _leftClick(self, event):
        ...
        if self.selected_idx == None and self.addable:
            self._addBar(x)
        else:
            if self.select_func is not None:
                self.select_func(self.selected_idx)
    ...
    def _addBar(self, x):
        if self.selected_idx == None:
            ...
            for bar in self.bars:
                bar["Ids"] = self.__addBar(bar["Pos"])
            if self.select_func is not None:
                self.select_func(len(self.bars) - 1)
    ...
    def __addBar(self, pos):
        ...
        id_outer = self.canv.create_rectangle(
            x - R,
            y - R,
            x + R,
            y + R,
        ...
        id_inner = self.canv.create_rectangle(
